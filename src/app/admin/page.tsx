...
export default function AdminPage() {
  ...
  const [walletProjects, setWalletProjects] =
    useState<any[]>([]);
  ...
  async function handleWalletSubmit() {
    if (!walletForm.title || !walletForm.contract) {
      alert('Title and Contract Address are required');
      return;
    }
    try {
      ...
      const walletData = {
        ...
        contract: walletForm.contract,
        chainId: Number(walletForm.chainId),
        tokenSymbol: walletForm.tokenSymbol,
        claimDate: walletForm.claimDate,
        createdAt: Date.now(),
      };
      if (editingWalletId) {
        await updateAirdrop(editingWalletId, walletData);
        alert('Wallet Project Updated Successfully');
      } else {
        await addAirdrop(walletData);
        alert('Wallet Project Saved Successfully');
      }
      resetWalletForm();
      await loadWalletProjects();
    } catch (error) {
      console.error(error);
      alert('Something went wrong');
    } finally {
  ...
  function handleWalletEdit(project: any) {
    setEditingWalletId(project.id);
    setWalletForm({
      title: project.title || '',
      contract: project.contract || '',
      chainId: String(project.chainId || ''),
  ...
  async function handleWalletDelete(id: string) {
    const confirmDelete = confirm('Delete this wallet project?');
  ...
  function handleEdit(project: any) {
    ...
    setForm({
      ...
      guide: Array.isArray(project.guide) ? project.guide.join('\n') : '',
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {activeTab === 'projects' && (
          <>
            ...
            >
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <input
                  ...
                  value={form.airdrop}
                  onChange={(e) => setForm({ ...form, airdrop: e.target.value })}
                  className="p-4 rounded-2xl bg-[#0d1326] border border-white/10 outline-none"
                />
                <input
                  type="text"
              ...
              <div className="flex items-center gap-4 bg-[#0d1326] border border-white/10 rounded-2xl p-5">
                <input
                  ...
                  className="w-5 h-5"
                />
                <p className="font-bold text-lg">Featured Project</p>
              </div>
            ...
            {/* PROJECT LIST */}
            <div className="mt-20">
              <h2 className="text-5xl font-black mb-10">Live Projects</h2>
                ...
                {projects.map((project) => (
                  ...
                  >
                    ...
                    <div className="p-6">
                      ...
                      <p className="text-gray-400 mt-4 line-clamp-3">{project.description}</p>
          ...
          <div className="space-y-8">
            <div className="bg-white/5 border border-white/10 p-10 rounded-[40px]">
              <h2 className="text-5xl font-black mb-10">
                {editingWalletId ? 'Edit Wallet Project' : 'Wallet Checker'}
              </h2>
              <div className="grid md:grid-cols-2 gap-6">
                <input
                  type="text"
                ...
                <input
                  ...
                  value={walletForm.contract}
                  onChange={(e) => setWalletForm({ ...walletForm, contract: e.target.value })}
                  className="p-4 rounded-2xl bg-[#0d1326] border border-white/10 outline-none"
                />
                <input
                  type="number"
                  ...
                  onChange={(e) => setWalletForm({ ...walletForm, chainId: e.target.value })}
                  className="p-4 rounded-2xl bg-[#0d1326] border border-white/10 outline-none"
                />
                <input
                  type="text"
                  placeholder="Token Symbol"
                  value={walletForm.tokenSymbol}
                  onChange={(e) => setWalletForm({ ...walletForm, tokenSymbol: e.target.value })}
                  className="p-4 rounded-2xl bg-[#0d1326] border border-white/10 outline-none"
                />
                <input
                  type="date"
                  value={walletForm.claimDate}
                  onChange={(e) => setWalletForm({ ...walletForm, claimDate: e.target.value })}
                  className="p-4 rounded-2xl bg-[#0d1326] border border-white/10 outline-none"
                />
              </div>
              ...
              <div className="flex gap-4 mt-8">
                <button
                  ...
                  className="flex-1 py-5 rounded-2xl bg-gradient-to-r from-green-500 to-emerald-600 font-black text-xl"
                >
                  {walletLoading ? 'PROCESSING...' : editingWalletId ? 'UPDATE WALLET PROJECT' : 'SAVE WALLET PROJECT'}
                </button>
                {editingWalletId && (
                  <button
                    type="button"
            ...
            {/* SAVED WALLET PROJECTS LIST */}
            <div>
              <h2 className="text-5xl font-black mb-10">Saved Wallet Projects</h2>
              {walletProjects.length === 0 ? (
                <div className="text-center text-gray-500 py-16 bg-white/5 border border-white/10 rounded-[32px]">
                  No wallet projects saved yet.
                </div>
              ) : (
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {walletProjects.map((wp) => (
                    <div
                      key={wp.id}
                      className="rounded-[32px] border border-white/10 bg-gradient-to-b from-[#111827] to-[#0b1020] p-6 space-y-4"
                    >
                      <div className="flex items-center justify-between">
                        <h3 className="text-2xl font-black">{wp.title}</h3>
                        <span className="px-4 py-1 rounded-full bg-green-500/10 border border-green-500/20 text-green-400 text-sm font-bold">
                          {wp.tokenSymbol || '—'}
                        </span>
                      </div>
                      <div className="text-gray-400 text-sm space-y-1">
                        <p>📅 Claim Date: <span className="text-white font-semibold">{wp.claimDate || '—'}</span></p>
                        <p className="truncate">📄 Contract: <span className="text-cyan-400 font-mono text-xs">{wp.contract || '—'}</span></p>
                        <p>🔗 Chain ID: <span className="text-white font-semibold">{wp.chainId || '—'}</span></p>
                      </div>
                      <div className="flex gap-3 pt-2">
                        <button
                          onClick={() => handleWalletEdit(wp)}
                          className="flex-1 py-3 rounded-2xl bg-blue-600 font-bold"
                        >
                          Edit
                        </button>
                        <button
                          onClick={() => handleWalletDelete(wp.id)}
                          className="flex-1 py-3 rounded-2xl bg-red-500 font-bold"
                        >
                          Delete
                        </button>
                      </div>
        ...
        {activeTab === 'blogs' && (
          <div className="space-y-12">
            ...
            >
              <h2 className="text-5xl font-black mb-10">Publish Blog</h2>
              <div className="grid md:grid-cols-2 gap-6">
                <input
                  type="text"
              ...
              <div className="flex items-center gap-4 bg-[#0d1326] border border-white/10 rounded-2xl p-5 mt-6">
                <input
                  ...
                  className="w-5 h-5"
                />
                <p className="font-bold text-lg">Featured Article</p>
              </div>
              <button
                type="submit"
              ...
              >
                {blogLoading ? 'PROCESSING...' : editingBlogId ? 'UPDATE BLOG' : 'PUBLISH BLOG'}
              </button>
            </form>

            {/* BLOG LIST */}
            <div>
              <h2 className="text-5xl font-black mb-10">Published Blogs</h2>
              <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-8">
                {blogs.map((blog: any) => (
                  ...
                  >
                    <img
                      src={blog.image || 'https://images.unsplash.com/photo-1639322537228-f710d846310a?q=80&w=1200&auto=format&fit=crop'}
                    ...
                    <div className="p-6">
                      ...
                      <p className="text-gray-400 mt-4 line-clamp-3">{blog.description}</p>
