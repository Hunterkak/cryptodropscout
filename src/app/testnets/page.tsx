...
export default function TestnetsPage() {
  ...
  useEffect(() => {
    ...
    getProjectsByCategory('testnet').then((data) => {
      setProjects(data);
      setLoading(false);
    });
  ...
  return (
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <Link
          href="/"
          className="text-sm text-gray-500 hover:text-cyan-400 mb-6 block"
        >
          ← Home
        </Link>
        ...
        <p className="text-gray-500 mb-10">
          Explore latest crypto testnet opportunities
        </p>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {projects.map((p: any) => (
              ...
              <div
                key={p.id}
                className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-cyan-500/50 transition-all"
              >
                ...
                <span className="absolute top-3 right-3 px-2 py-0.5 text-xs font-bold rounded-full bg-black/60 text-cyan-400 border border-cyan-400/30">
                  {p.reward}
                </span>
